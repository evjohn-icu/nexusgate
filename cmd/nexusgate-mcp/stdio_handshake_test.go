...
func TestStdioHandshake(t *testing.T) {
	...
	bin := filepath.Join(t.TempDir(), "nexusgate-mcp")
	build := exec.Command("go", "build", "-o", bin, "./cmd/nexusgate-mcp")
	build.Dir = "../.."
	if out, err := build.CombinedOutput(); err != nil {
		t.Fatalf("build nexusgate-mcp: %v\n%s", err, out)
	}

	// The binary refuses to start without a pinned Hub identity when
	// NEXUSGATE_BASE_URL is unset (it defaults to https), so give it an
	// explicit loopback http base URL, which is the TLS-off development form
	// that needs no fingerprint.
	cli, err := client.NewStdioMCPClient(bin, []string{"NEXUSGATE_BASE_URL=http://127.0.0.1:8787"})
	if err != nil {
		t.Fatal(err)
	}
	defer cli.Close()
	...
	if info.ServerInfo.Name != "nexusgate" {
		t.Fatalf("server name = %q, want nexusgate", info.ServerInfo.Name)
	}
