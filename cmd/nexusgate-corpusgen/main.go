// Command nexusgate-corpusgen generates the offline eval corpus for
// nexusgate-eval: deterministic ffmpeg clips plus ground_truth.json in the
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, `nexusgate-corpusgen — generate the offline eval corpus for nexusgate-eval
...
mirror the retrieval golden set's adversarial assets, so nexusgate-eval scores a
provider the way the Hub's own search would. Never part of CI.

Usage:
  nexusgate-corpusgen --out <dir>
...
`)
	...
	if err := generate(*out); err != nil {
		fmt.Fprintf(os.Stderr, "nexusgate-corpusgen: %v\n", err)
		os.Exit(1)
	}
