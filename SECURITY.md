...
Re:Footage (`nexusgate`) holds third-party provider API keys in an encrypted
...
<https://github.com/evjohn-icu/nexusgate/security/advisories/new>
...
NexusGate is **local-first**: one trusted machine on a trusted LAN, running the
...
- Reading `$NEXUSGATE_DATA_DIR` from a backup, a snapshot, a synced folder, or a
...
- Vulnerabilities in `ffmpeg`, `ffprobe`, `exiftool`, or a local VLM/aligner you
  configured — report those upstream. A bug in *how NexusGate invokes them* is
...
- If you run the Hub in Docker with a published port, or behind a reverse proxy,
  ...
  reach the port.
- Keep `$NEXUSGATE_DATA_DIR` on a filesystem that honours Unix permissions, and
