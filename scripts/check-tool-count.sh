...
# registrations in cmd/timingdex-mcp/main.go and every manifest/doc that
...
main_go="$root/cmd/timingdex-mcp/main.go"
...
do
  ...
  printf '%s claims %s tools, but %s registers %s (%s) via AddTool(\n' "$doc" "$claim_lc" "cmd/timingdex-mcp/main.go" "$registered" "$word" >&2
  failures=$((failures + 1))
done
