...
- **New files under `cmd/nexusgate-mcp/` are visible to git again** (audit
  G1-07).
  `.gitignore` carried a bare `nexusgate-mcp` for the built binary, which also
  matched the `cmd/nexusgate-mcp` *directory*, so every new file added there
  ...
  to fail. Anchored to `/nexusgate-mcp` (and `/nexusgate-corpusgen`, which had
  the same problem), matching the `/nexusgate` two lines above it that was
  always right.
...
- **`/setup` names each failed check and what to do about it** (audit U2-02).
  ...
  — not which remedy applies, and not that `nexusgate doctor` prints the same
...
- **Documentation corrected against the code it describes** (2026-08-29 audit,
  ...
  - `skills/nexusgate/` and the MCP reference no longer promise that the agent
  ...
  - The README and `deploy/unraid/README.md` claimed a container deployment
    ...
    both ship `NEXUSGATE_HUB_ADMIN_AUTH=required`, both admin-auth settings are
...
- **Clean container startup by default**: the shipped Docker Compose and Unraid
  Hub entry points now default `NEXUSGATE_HUB_ADMIN_AUTH=required`, so a fresh
  ...
  RFC1918 address. `NEXUSGATE_HUB_ADMIN_AUTH_NETWORKS` is split and validated
...
- **Pagination and resource semantics**: Search responses report
  ...
  accept optional `offset` and answer with `X-NexusGate-Limit`/
  `X-NexusGate-Offset`/`X-NexusGate-Has-More`; parent/child endpoints distinguish
  ...
  envelope exposing the stable `code`. An unset `NEXUSGATE_BASE_URL` now
  defaults to `https://127.0.0.1:8787` (requiring `NEXUSGATE_HUB_FINGERPRINT`
...
- **Generated Worker installer works against the default self-signed Hub**: the
  ...
  after an `nexusgate:admin-auth-changed` login.

- **CLI usage synopsis**: `nexusgate` help now enumerates
...
- **Browser multilingual UI**: the 11 inline HTML pages (library, setup,
  ...
  shared sidebar writes a non-sensitive `nexusgate_locale` preference cookie
...
- **Footage capability provider**: NexusGate now exposes its library to
...
- **MCP**: `nexusgate-mcp` gains a sixth tool `get_transcript(asset_id)`,
  plus certificate fingerprint pinning for cross-machine Hubs:
  `NEXUSGATE_HUB_FINGERPRINT` (printed by `nexusgate serve`) pins the Hub's
  ...
  `plugins/claude/`) that declares `nexusgate-mcp` over stdio and passes
  through `NEXUSGATE_BASE_URL`, `NEXUSGATE_AGENT_TOKEN` and
  `NEXUSGATE_HUB_FINGERPRINT`; the plugin ships no binary.
...
- Analysis now enqueues an idempotent `JobIndex` successor after every successful analyzer
  ...
  Added offline `nexusgate search rebuild` to repair all assets with canonical analysis or
...
- **Real-machine deployment hardening (107 / NAS / agent-plan)**:
  - **Fail-fast provider validation**: `nexusgate serve`/`pipeline run` now refuse to start
  ...
  - 部署运维要点（SSH 里用 `pkill -x nexusgate`、`/tmp` 满、key 环境变量、`pipeline run`
...
- **素材目录状态表加「提示」列**：`/library-roots` 的健康表对每个已添加根显示
  `nexusgate doctor` 同源的存储建议（网络挂载提示、SMB/NFS 根建议 `source_staging.mode
...
- 设置 API 在一个版本内兼容读取旧的 `daily_budget` 与 `monthly_budget` 字段，
  ...
  新流程不再产生。
- **缓存维护 CLI**：新增 `nexusgate cache inspect`、`gc`、`verify` 与
...
- **分析提交与 model run 一致性**：model run 的创建、阶段转换和 canonical
  ...
  重建可安全执行。
- **搜索索引维护**：分析成功后幂等地排队 `JobIndex`，`nexusgate search rebuild`
...
- **Embedding 防护**：文本 embedding 拒绝维度错误、非有限值和无效向量，重建
  ...
  embedding 仍只是 retrieval 信号而不是 evidence。`nexusgate search
...
- **自动增量 + 全量重建**：pipeline 提交后 hook（分析/精修两个提交点）
  ...
  `nexusgate search rebuild-embeddings` 全量重建。换 embedding 模型 =
...
- **eval 工具修复**：`nexusgate-eval score` 的位置索引 flag 校验 bug
  （传 `--labels` 却检查 `label`，导致正确调用报 "--labels is required"）
  重写为显式校验 + 回归测试；`score` 输出新增按信号归因的
  semantic/lexical false positives。新增 `cmd/nexusgate-corpusgen`
...
Roadmap 第一阶段：把 shot 时间轴从 LLM 手里拿出来。NexusGate 负责检测与采样，
...
- **`openai_multiframe` 协议**：`local_vlm.protocol` 支持
  ...
  chat/completions 表面）。端点永不接收整段视频；NexusGate 对每个 shot 采样
...
- **确定性 shot 检测**：`shot_detection` 配置块，两种 detector——
  ...
  forced aligner 同款）与内置 `ffmpeg_scene`（零新依赖）。NexusGate 硬校验：
...
- **eval 工具**：`cmd/nexusgate-eval`（内部 `internal/eval`）——真实 clips 进
...
使用：`nexusgate-eval run --corpus ./corpus --data-dir ./eval/qwen --label
qwen3vl-4b && nexusgate-eval score --corpus ./corpus --data-dir ./eval
...
- **P1 facet 语义**：shot search 的 shot_size/camera_motion/audio_type/quality/
  ...
  现在是文档化语义而非 bug。
- **P1 reanalysis 机制（新增）**：`nexusgate reanalyze --asset <id> | --root <id> |
...
**旧素材需要 `nexusgate reanalyze --all`** 才能吃到 v4 prompt 的新 shot 语义
与时间轴逻辑。
...
Docker 镜像 tag：`nexusgate:v0.24.1`。
...
- **测试覆盖**：cmd/nexusgate 17.4%→55.9%（run() 子命令分发 table-driven）；
  internal/remote 新增 12 个 wire type 往返测试。

Docker 镜像 tag：`nexusgate:v0.24.0`。
...
Docker 镜像 tag：`nexusgate:v0.23.1`。

## v0.23.0 — 2026-08-05（剪辑 agent 接入：MCP + 按需 WebDAV 交付）

剪辑 agent 接入的两层能力：`cmd/nexusgate-mcp`（MCP 语义检索，Codex 等客户端
可接入）+ 按需 WebDAV 交付空间（素材经软链按需可见、只读、流式）。

### MCP server（cmd/nexusgate-mcp）：独立 stdio 二进制，Codex 等 MCP 客户端
...
- **测试覆盖**：`internal/ingest`（无人值守扫描核心）覆盖率 3.5% → **82.4%**
  ...
  `cmd/nexusgate` 0% → **14%+**（usage/parseWorkerMounts/repeatedFlag/hubTLSFiles
...
- **secretstore Rekey CLI**：新增 `nexusgate secrets rekey`——轮换数据加密密钥、
...
- **结构化日志**：`NEXUSGATE_LOG_FORMAT`（text|json，默认 text）+
  `NEXUSGATE_LOG_LEVEL`（debug|info|warn|error，默认 info）环境变量配置
  slog handler；无变量时行为与默认一致，非法值回退并告警。
- **SQLite integrity**：`nexusgate doctor` 增加 `PRAGMA integrity_check`，
  健康库输出 `sqlite: ok`，损坏库报告错误并非零退出。

## v0.21.0 — 2026-08-04（首个 GitHub release）

合并 `hardware-and-mounts` 全量（v0.20 NAS 挂载 + v0.21 无人值守巡检/时间线导出，此前均未发布）与 2026-08-04 全库审查批（OpenSpec 落地、API 硬化、model_runs 边界、21 个 change，见下方对应小节）。Docker 镜像 tag：`nexusgate:v0.21.0`。
...
- `nexusgate serve` can now rescan every library root on a timer and drain the
...
- Both routes require the Hub administrator token and reject the agent token,
  ...
  `skills/nexusgate/references/api-contract.md` says the same thing, so a
...
- Added commented-out device-passthrough stanzas to both `hub` and `worker`
  ...
  `ports`, `NEXUSGATE_BIND`, the read-only media binds, the Worker's
...
- Added `deploy/unraid/nexusgate-hub.xml` and `deploy/unraid/nexusgate-worker.xml`,
...
- Added `nexusgate worker run --tray`: a Windows notification-area icon with a
...
- Added a way back for failed jobs: `nexusgate pipeline retry-failed`, and a
...
- Added optional `source_staging.mode: copy` for mounted NAS/network libraries.
  NexusGate copies a source to a local, versioned cache before processing and
...
- Added a NAS/local-source choice to the startup-command page via
  `NEXUSGATE_SOURCE_STAGING_MODE=copy`.

## v0.13.0 — Library-First Footage Browser

- Reframed `/` as the primary NexusGate experience: an editor now sees each
...
- Added a project-bundled `nexusgate` Agent Skill for local API-only readiness
...
- Added FFmpeg capability detection and an inspectable `GET /api/v1/hardware`
  endpoint plus `nexusgate doctor` output.
