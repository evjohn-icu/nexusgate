# NexusGate v0.15 Local Agent API Contract

Base URL: the user’s local NexusGate server, normally
...
NexusGate has two independent bearer credentials, generated once on first Hub
...
command `nexusgate root scan <root-id>` uses the same scan path but runs the
...
reserved for a human in the NexusGate workspace, and the agent token is refused
