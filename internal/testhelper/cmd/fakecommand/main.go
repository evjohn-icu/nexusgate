...
func main() {
	...
	case "ffprobe":
		output := ffprobeJSON
		if transfer := os.Getenv("TIMINGDEX_FAKE_FFPROBE_COLOR_TRANSFER"); transfer != "" {
	...
	case "align-empty":
		fmt.Print(`{"words":[]}`)
	default:
		fmt.Fprintf(os.Stderr, "unknown timingdex test command %q", name)
		os.Exit(2)
	}
}

func fakeFFmpeg() {
	failHardware := os.Getenv("TIMINGDEX_FAKE_FFMPEG_FAIL_HARDWARE") == "1"
	for i, arg := range os.Args[1:] {
		if failHardware && (arg == "-hwaccel" || arg == "h264_nvenc" || strings.Contains(arg, "h264_nvenc")) {
			os.Exit(1)
		}
		if arg == "-vf" && i+2 <= len(os.Args[1:]) {
			if logPath := os.Getenv("TIMINGDEX_FILTER_LOG"); logPath != "" {
