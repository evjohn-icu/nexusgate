...
// Timingdex-owned service. The VLM must never be the one to decide where a
// shot begins and ends — it is asked to describe pixels, and Timingdex is the
...
// Timingdex-owned rules are enforced: monotonic, non-overlapping, inside the
...
// Normalize enforces the Timingdex-owned shot rules on raw detector output
