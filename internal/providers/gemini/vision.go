...
func defaultPrompt(req videoanalysis.Input) string {
	...
	return `Analyze this footage and return one JSON object only. Use this unified schema: summary (string), scenes (array of {description,start_ms,end_ms,tags}), objects (array of {name,count,confidence}), actions (array of {name,confidence}), mood (array of strings), raw_tags (array of strings), shots (array of {start_ms,end_ms,description,tags,objects,actions,mood,confidence}), confidence (number). Also include analysis with the legacy fields asset_type, scene_tags, subjects, people_count, shot_size, camera_motion, lighting, audio_type, has_speech, summary, usable_as, mood_tags, quality, quality_flags, extra_tags, editorial_reason. ` + normalize.VocabularyPrompt() + normalize.OutputLanguagePrompt(req.Language) + `Metadata: ` + string(meta) + ` Transcript: ` + tr
}
