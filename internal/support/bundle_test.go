...
func TestSupportBundleRedactsSecretsAndBasenames(t *testing.T) {
	...
	cfg := config.Config{
		DataDir:       "/srv/nexusgate",
		CacheDir:      "/srv/nexusgate/cache",
		DatabasePath:  "/srv/nexusgate/nexusgate.db",
		...
		Providers: config.ProvidersConfig{
			...
			Alignment: config.AlignmentConfig{
				Command: "/opt/nexusgate/bin/nexusgate-align",
		...
		HubTLS:   config.HubTLSConfig{Mode: "files", CertificateFile: "/etc/nexusgate/tls.crt", KeyFile: "/etc/nexusgate/tls.key"},
	...
	report := domain.DoctorReport{
		...
		DB:       domain.DBInfo{Path: "/srv/nexusgate/nexusgate.db", IntegrityOK: true, MigrationsApplied: 29, MigrationsTotal: 29, SchemaVersion: "0029_cost_ledger.sql"},
		Storage:  domain.StorageInfo{CachePath: "/srv/nexusgate/cache", FreeDiskBytes: 1 << 40, CacheBytes: 1 << 30, DBSizeBytes: 1 << 20},
	...
	if got := sanitized["data_dir"]; got != "nexusgate" {
		t.Errorf("config.sanitized.json data_dir = %v, want basename nexusgate", got)
	}
	if got := sanitized["cache_dir"]; got != "cache" {
		t.Errorf("config.sanitized.json cache_dir = %v, want basename cache", got)
	}
	if got := sanitized["database_path"]; got != "nexusgate.db" {
		t.Errorf("config.sanitized.json database_path = %v, want basename nexusgate.db", got)
	}
	...
	if got := align["command"]; got != "nexusgate-align" {
		t.Errorf("config.sanitized.json alignment.command = %v, want basename nexusgate-align", got)
	}
	...
	if got := bundle.Report.DB.Path; got != "nexusgate.db" {
		t.Errorf("bundle report db.path = %q, want basename nexusgate.db", got)
	}
