...
// admin/agent token, or a valid, unredeemed X-Timingdex-Pairing-Token header —
...
func TestWorkerBootstrapBinaryAuth(t *testing.T) {
	...
	remoteRequest := func(header string) *http.Request {
		...
		if header != "" {
			request.Header.Set("X-Timingdex-Pairing-Token", header)
		}
		return request
	}
...
func remoteRequestWithToken(target, token string) *http.Request {
	...
	request.RemoteAddr = "203.0.113.50:9999"
	request.Header.Set("X-Timingdex-Pairing-Token", token)
	return request
}
...
func TestGeneratedWorkerScriptPinsAndVerifiesBinary(t *testing.T) {
	...
	for _, platform := range []string{"linux-amd64", "windows-amd64"} {
		t.Run(platform, func(t *testing.T) {
			...
			// private .timingdex-worker/worker.json, not the old top-level file.
			if strings.Index(script, "--fingerprint") > strings.Index(script, "--pairing") {
				t.Fatalf("--fingerprint must appear before --pairing:\n%s", script)
			}
			if !strings.Contains(script, "--config ./.timingdex-worker/worker.json") {
				t.Fatalf("script does not use the private config path:\n%s", script)
			}
			if !strings.Contains(script, "worker run --config ./.timingdex-worker/worker.json") || !strings.Contains(script, "worker doctor --config ./.timingdex-worker/worker.json") {
				t.Fatalf("run/doctor instructions must use the private config path:\n%s", script)
			}
			if strings.Contains(script, "timingdex-worker.json") {
			...
			if platform == "linux-amd64" {
				for _, marker := range []string{
					...
					"timingdex-linux-amd64.download",
				} {
				...
				for _, marker := range []string{
					...
					"timingdex-windows-amd64.exe.download",
				} {
					if !strings.Contains(script, marker) {
...
func TestWorkerSetupScriptGenerationRequiresBinaryAndTLS(t *testing.T) {
	...
	t.Run("binary but no TLS", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(binDir, "timingdex-linux-amd64"), []byte("binary"), 0o755); err != nil {
			t.Fatal(err)
		}
		handler := NewServer("", service).Handler()
		response := httptest.NewRecorder()
...
func TestWorkerSetupRetriesContextAfterLogin(t *testing.T) {
	...
	authChanged := `window.addEventListener('timingdex:admin-auth-changed',function(event){if(event.detail&&event.detail.authenticated){loadContext()}else{ctx=ctx||{};ctx.library_roots=(ctx.library_roots||[]).map(function(root){return{id:root.id}});renderMounts()}});`
...
// smoke for this bootstrap flow: it builds the real timingdex binary, serves
...
// .timingdex-worker directory). The pinned fingerprint must authenticate the
...
func TestGeneratedScriptEnrollsCompiledBinaryAgainstLocalTLSHub(t *testing.T) {
	...
	if err != nil {
		t.Skip("go toolchain unavailable to build the timingdex binary")
	}
	workDir := t.TempDir()
	binPath := filepath.Join(workDir, "timingdex-smoke")
	build := exec.Command(goTool, "build", "-o", binPath, "github.com/evjohn-icu/timingdex/cmd/timingdex")
	if out, err := build.CombinedOutput(); err != nil {
		t.Fatalf("building timingdex failed: %v\n%s", err, out)
	}
	...
	configPath := filepath.Join(workDir, ".timingdex-worker", "worker.json")
	...
	enroll.Env = append(os.Environ(), "TIMINGDEX_WORKER_CONFIG="+configPath)
	...
	second := exec.Command(binPath, "worker", "enroll",
		...
		"--fingerprint", fingerprint,
		"--pairing", pairing.Token,
		"--config", filepath.Join(workDir, ".timingdex-worker", "second.json"),
	)
