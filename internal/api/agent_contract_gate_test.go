...
// search_relevance_eval_test.go uses, so the gate resolves skills/nexusgate/
...
func TestAgentCapabilitiesMatchesSkillMD(t *testing.T) {
	...
	skillPath := filepath.Join(root, "skills/nexusgate/SKILL.md")
...
func TestAgentCapabilitiesMatchesAPIContractDoc(t *testing.T) {
	...
	root := agentContractGateRepoRoot(t)
	docPath := filepath.Join(root, "skills/nexusgate/references/api-contract.md")
