...
func searchBarHTML() string {
	return `<form class="search-bar" role="search" onsubmit="search(event)"><input id="q" name="q" placeholder="搜镜头内容、说了什么或标签" aria-label="镜头搜索"><button type="submit" class="search-submit">搜索</button><button type="button" class="search-clear" onclick="clearSearch()">清除</button></form>`
}
