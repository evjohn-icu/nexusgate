...
func (s *Server) workerSetupPage(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(brandedPage(workerSetupPageHTML)))
}
...
<header><span class="brand">Timingdex</span><a href="/">素材库</a><a href="/workers">节点管理</a><strong style="color:#b8c8ff;font-size:14px">Worker 安装向导</strong><input id="admin-token" type="password" autocomplete="off" placeholder="Hub 管理 Token（仅存于本页内存）"></header>
<div class="wrap">
