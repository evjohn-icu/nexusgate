...
async function init(){try{var contextResponse=await fetch('/api/v1/hub/worker-setup/context');if(!contextResponse.ok)throw new Error(await apiErrMsg(contextResponse));ctx=await contextResponse.json();var t=adminToken();if(t){var rootsResponse=await fetch('/api/v1/admin/hub/worker-setup/library-roots',{headers:{Authorization:'Bearer '+t}});if(!rootsResponse.ok)throw new Error(await apiErrMsg(rootsResponse));var details=await rootsResponse.json();ctx.library_roots=details.library_roots||[]}var hubInfo=document.getElementById('hub-info');hubInfo.innerHTML='<div class="info-line"><span class="info-label">Hub URL</span><span class="info-value">'+esc(ctx.hub_url)+'</span></div><div class="info-line"><span class="info-label">TLS</span><span class="info-value '+(ctx.tls?'tls-on':'tls-off')+'">'+(ctx.tls?'已启用':'未启用')+'</span></div>'+(ctx.fingerprint?'<div class="info-line"><span class="info-label">证书指纹</span><span class="fingerprint">'+esc(ctx.fingerprint)+'</span></div>':'')+'<div class="info-line"><span class="info-label">素材目录</span><span class="info-value">'+(ctx.library_roots||[]).length+' 个</span></div>';renderBinaries();renderMounts()}catch(e){document.getElementById('hub-info').innerHTML='<div class="muted">无法获取环境信息：'+esc(e.message)+'</div>'}}
