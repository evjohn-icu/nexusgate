...
func TestAPIErrorFromErrorSentinelClasses(t *testing.T) {
	tests := []struct {
		...
		{"route unconfirmed -> 503 retryable", fmt.Errorf("%w: %w", providerchannels.ErrRouteUnconfirmed, errors.New("last non-terminal failure")), "provider_route_unconfirmed", true, true},
...
func TestAPIErrorFromErrorStatusCodes(t *testing.T) {
	tests := []struct {
		...
		{providerchannels.ErrRouteUnconfirmed, http.StatusServiceUnavailable},
