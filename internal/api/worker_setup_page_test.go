...
func TestWorkerSetupPageRendersWithStepMarkers(t *testing.T) {
	...
	for _, marker := range []string{
		`<title>NexusGate · [[i18n:workerSetup.title]]</title>`,
...
func TestWorkerSetupScriptPowerShellIncludesErrorActionPreference(t *testing.T) {
	...
	if !strings.Contains(script, "nexusgate-windows-amd64.exe") {
...
func TestWorkerSetupBinaryDownloadSuccess(t *testing.T) {
	...
	binaryContent := []byte("fake-nexusgate-binary")
...
func TestWorkerSetupContextWithBinaryPresent(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(binDir, "nexusgate-linux-amd64"), []byte("binary-content"), 0o755); err != nil {
		t.Fatal(err)
	}
	service, err := app.NewService(repo, config.Config{DataDir: dataDir, Hardware: media.HardwareConfig{Mode: "software", AllowFallback: true}})
	if err != nil {
...
func TestWorkerSetupBinaryDownloadServesCorrectFilePerPlatform(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(binDir, "nexusgate-linux-amd64"), []byte("linux-amd64-content"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(binDir, "nexusgate-linux-arm64"), []byte("linux-arm64-content"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(binDir, "nexusgate-windows-amd64.exe"), []byte("windows-content"), 0o755); err != nil {
		t.Fatal(err)
	}
	service, err := app.NewService(repo, config.Config{DataDir: dataDir, Hardware: media.HardwareConfig{Mode: "software", AllowFallback: true}})
	if err != nil {
