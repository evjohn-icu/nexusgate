...
function tdPlural(key,count,vars){var rule='other';try{rule=new Intl.PluralRules(window.TD_LOCALE).select(Number(count)||0)}catch(_){}var full=key+'.'+rule;var s=_tdLookup(full);if(s==null)s=_tdLookup(key+'.other');if(s==null)s=key;var all={count:count};if(vars){for(var k in vars){all[k]=vars[k]}}for(var k in all){s=s.split('{'+k+'}').join(String(all[k]))}return s}
