...
func (s *Server) settingsPage(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write([]byte(brandedPage(settingsHTML)))
}
...
if(d.holding_above>0){pills.push('<span class="pill hold">正在推迟 &gt;'+Math.round(d.holding_above/MB)+' MB 的素材，'+esc(d.next_off_peak)+' 开始处理</span>')}
