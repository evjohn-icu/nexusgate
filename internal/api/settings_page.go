...
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:var(--ui-bg);color:var(--ui-text);font:14px ui-sans-serif,system-ui,-apple-system,sans-serif}header{padding:15px 5vw;border-bottom:1px solid var(--ui-border);display:flex;gap:18px;align-items:center;background:color-mix(in srgb, var(--ui-bg) 93%, transparent);position:sticky;top:0;z-index:1;backdrop-filter:blur(12px)}a{color:#b8c8ff;text-decoration:none}.brand{color:#fff;font-weight:800;margin-right:auto}header input{width:auto;max-width:260px;padding:9px 10px;border-radius:8px;border:1px solid #354764;background:var(--ui-surface-inset);color:#fff;font:inherit}.wrap{max-width:900px;margin:auto;padding:32px 24px 64px}h1{font-size:32px;margin:0 0 6px;letter-spacing:-.04em}.muted{color:var(--ui-text-muted);line-height:1.6;margin:0}.panel{background:var(--ui-surface-raised);border:1px solid var(--ui-border);border-radius:14px;padding:20px;margin-top:20px}.panel h2{margin:0 0 6px;font-size:17px}.panel .hint{color:#9fb0ce;font-size:13px;line-height:1.6;margin:0 0 16px}.row{display:grid;grid-template-columns:220px 1fr;gap:14px;align-items:start;padding:12px 0;border-top:1px solid #24344e}.row:first-of-type{border-top:0}label{font-weight:700;color:#dce6fb}.row .note{color:#93a5c3;font-size:12px;line-height:1.5;margin:6px 0 0}input[type=number],input[type=text],input[type=time]{width:170px;padding:9px 10px;border-radius:8px;border:1px solid #354764;background:var(--ui-surface-inset);color:#fff;font:inherit}input[type=checkbox]{width:17px;height:17px;accent-color:#86a3ff}button{background:#86a3ff;color:#091227;border:0;border-radius:9px;padding:11px 16px;font-weight:800;cursor:pointer}button.ghost{background:#31446a;color:#dbe6ff}.actions{display:flex;gap:10px;align-items:center;margin-top:22px}.status{border-radius:10px;padding:12px 14px;margin-top:18px;line-height:1.6;display:none}.status.ok{display:block;background:#153c31;color:#a4efc8;border:1px solid #23664f}.status.bad{display:block;background:#3f2230;color:#ffc2cc;border:1px solid #6d3547}.state{display:flex;flex-wrap:wrap;gap:10px;margin-top:4px}.pill{border-radius:99px;padding:5px 11px;font-size:12px;font-weight:700;background:#243651;color:#c3d3f2}.pill.live{background:#153c31;color:#a4efc8}.pill.hold{background:#43331c;color:#ffd79a}@media(max-width:720px){.row{grid-template-columns:1fr}}</style></head><body><!--SHELL_HEADER--><main class="wrap">
<h1>设置</h1>
<p class="muted">这里的改动立即生效，不用重启 Hub——处理流程每次开工前都会重读限制，正在跑的任务也会马上被约束。</p>
<section class="panel"><h2>存储概览</h2><p class="hint">「原始素材」一行是<b>估算（只读）</b>：来自入库时记的体积，不是实时扫描——原始素材永远留在素材库盘上，Hub 只读。其余都在 Hub 缓存盘上：预览视频、缩略图、音频都可以从原始素材重新生成，<b>删了能再生成</b>；临时文件随时可删。</p><div id="storage-rows" class="state"><span class="pill">正在读取…</span></div></section>
