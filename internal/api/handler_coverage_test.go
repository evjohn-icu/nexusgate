...
func TestPreviouslyUncoveredHandlers(t *testing.T) {
	...
	tests := []struct {
		...
		{"collection assets", http.MethodGet, "/api/v1/collections/missing/assets", []int{http.StatusNotFound}},
		{"delete collection", http.MethodDelete, "/api/v1/collections/missing", []int{http.StatusNoContent, http.StatusNotFound}},
		{"asset shots", http.MethodGet, "/api/v1/assets/missing/shots", []int{http.StatusNotFound}},
