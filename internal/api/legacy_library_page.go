...
.shot-lines{list-style:none;margin:8px 0 0;padding:0;display:flex;flex-direction:column;gap:4px}
.shot-line{display:flex;gap:10px;align-items:baseline;font-size:12px;line-height:1.5}
.shot-line-time{flex:none;font-family:var(--font-data);font-variant-numeric:tabular-nums;font-size:11px;color:var(--text-faint)}
.shot-line-text{color:var(--text);min-width:0;overflow-wrap:anywhere}
...
function timeline(x,shots){const duration=Math.max(Number(x.duration_ms)||0,1);if(!shots.length)return '<div class="timeline-empty">'+tdT('library.timelineEmpty')+'</div>';const labels=['00:00',fmt(duration/2),fmt(duration)];const blocks=shots.map(s=>{const start=Math.max(0,Number(s.start_ms)||0),end=Math.max(start,Number(s.end_ms)||start),left=Math.min(100,start/duration*100),width=Math.max(1,(end-start)/duration*100),description=s.description||((s.tags||[]).join(' · '))||tdT('library.unnamedShot'),title=fmt(start)+' — '+fmt(end)+' · '+description;return '<div class="tickrule-span is-possible" style="left:'+left.toFixed(3)+'%;width:'+width.toFixed(3)+'%" title="'+esc(title)+'"></div>'}).join('');const lines=shots.map(s=>{const start=Math.max(0,Number(s.start_ms)||0),end=Math.max(start,Number(s.end_ms)||start),description=s.description||((s.tags||[]).join(' · '))||tdT('library.unnamedShot');return '<li class="shot-line"><span class="shot-line-time">'+fmt(start)+'–'+fmt(end)+'</span><span class="shot-line-text">'+esc(description)+'</span></li>'}).join('');return '<div class="tickrule" aria-label="'+tdT('library.timelineAria')+'"><div class="tickrule-track">'+blocks+'</div><div class="tickrule-scale"><span>'+labels[0]+'</span><span>'+labels[1]+'</span><span>'+labels[2]+'</span></div><ol class="shot-lines">'+lines+'</ol></div>'}
