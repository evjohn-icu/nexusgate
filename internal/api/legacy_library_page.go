...
async function load(ids){library.innerHTML='<div class="loading">'+tdT('library.loadingTimeline')+'</div>';try{let data=await fetch('/api/v1/assets?limit=300').then(r=>r.ok?r.json():[]);data=Array.isArray(data)?data:[];if(ids)data=data.filter(x=>ids.includes(x.id));if(!data.length){library.innerHTML='<div class="empty">'+esc(tdT('library.emptyTitle'))+'<a href="/setup" style="color:#b7c8eb;text-decoration:underline">'+esc(tdT('library.emptySetupLink'))+'</a>'+esc(tdT('library.emptyTail'))+'<br><a href="/library-roots" style="display:inline-block;margin-top:14px;background:#324767;color:#eaf1ff;border-radius:9px;padding:10px 13px;font-weight:750">'+esc(tdT('library.emptyRootsLink'))+'</a></div>';return}const rows=await Promise.all(data.map(async x=>row(x,await loadShots(x.id))));library.innerHTML=rows.join('')}catch(e){library.innerHTML='<div class="empty error">'+esc(tdT('library.loadFailed'))+esc(e.message)+'</div>'}}
