...
<title>NexusGate · [[i18n:setup.title]]</title>
...
function setupLoadStatus(){
  ...
  fetch('/api/v1/setup/status').then(function(r){if(!r.ok)throw Error(String(r.status));return r.json()}).then(function(s){
    ...
    const envFixes=[];
    function needsFix(ok,label,how){if(!ok)envFixes.push([label,how])}
    ...
    needsFix(!!s.ffmpeg,'FFmpeg',tdT('setup.env.fixFfmpeg'));
    needsFix(!!s.ffprobe,'FFprobe',tdT('setup.env.fixFfprobe'));
    needsFix(!!s.data_dir_writable,tdT('setup.env.dataDirWritable'),tdT('setup.env.fixDataDir'));
    needsFix(!!s.cache_writable,tdT('setup.env.cacheWritable'),tdT('setup.env.fixCache'));
    needsFix(!!s.free_disk_ok,tdT('setup.env.diskSpace'),tdT('setup.env.fixDisk'));
    needsFix(!!s.db_healthy,tdT('setup.env.dbHealth'),tdT('setup.env.fixDb'));
    const fixBox=document.getElementById('env-fixes');
    if(fixBox){fixBox.innerHTML=envFixes.length?'<div class="callout callout--contradicted" role="alert"><span><b>'+esc(tdT('setup.env.fixTitle'))+'</b><ul class="env-fix-list">'+envFixes.map(function(f){return '<li><b>'+esc(f[0])+'</b> — '+esc(f[1])+'</li>'}).join('')+'</ul></span></div>':''}
    setupPill('pill-env',envFixes.length===0);
    ...
    if(box){if(envFixes.length){box.className='status bad';box.textContent=tdPlural('setup.status.envIssues',envFixes.length)}else{box.className='status ok';box.textContent=tdT('setup.status.updated')}}
  }).catch(function(){
