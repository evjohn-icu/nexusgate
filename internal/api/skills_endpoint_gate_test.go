...
// This gate exists because skills/nexusgate/ restates the route table by
...
// skills/nexusgate/ is invisible unless someone remembers to grep for it.

// docEndpoint is one endpoint mention found under skills/nexusgate/, already
...
type docEndpoint struct {
	method string
	path   string
	file   string // path relative to skills/nexusgate/, for failure messages
}
...
// extractDocumentedEndpoints scans every file under skills/nexusgate/ for
...
// either be documented somewhere under skills/nexusgate/, or be named here
...
// /api/v1/... endpoint mentioned anywhere under skills/nexusgate/ must exist
...
func TestSkillsDocumentedEndpointsExistInRouteInventory(t *testing.T) {
	...
	skillsDir := filepath.Join(root, "skills/nexusgate")
...
// agent-reachable route must be either documented under skills/nexusgate/ or
...
func TestAgentReachableRoutesAreDocumentedOrAllowlisted(t *testing.T) {
	...
	for _, spec := range specs {
		...
		t.Errorf("agent-reachable route %q (auth=%s) is not documented anywhere under skills/nexusgate/ and is not in undocumentedAgentRoutes; document it or add an allow-list entry with a reason", key, spec.Auth)
	}
