...
func TestLibraryRootsPageOffersLoginOnAuthFailure(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`nexusgate:admin-auth-changed`, // logging in retries the step
...
// `nexusgate doctor` prints the same probes with the paths this page
...
func TestSetupPageNamesEveryFailedCheck(t *testing.T) {
	...
	for _, key := range []string{"setup.env.fixDataDir", "setup.env.fixCache", "setup.env.fixDisk"} {
		if v := catalog[key]; strings.Contains(v, "/") && !strings.Contains(v, "nexusgate cache gc") {
