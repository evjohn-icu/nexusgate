...
.coll-head{display:flex;justify-content:space-between;gap:14px;align-items:center;padding:15px 18px;cursor:pointer;width:100%;border:0;background:transparent;color:inherit;font:inherit;text-align:left}
.coll-card.open .coll-head{border-bottom:1px solid var(--rule)}
...
.coll-head{display:grid;grid-template-columns:minmax(0,1fr);gap:7px;align-items:start}
...
<h1>收藏</h1>
<p class="muted">在素材库的搜索结果里，把值得留的镜头收进收藏篮，随时回看或导出时间标记。</p>
...
function collectionsRender(list){const el=document.getElementById('collections');if(!list.length){el.innerHTML='<div class="coll-empty">还没有收藏。在素材库的搜索结果里把镜头加入收藏。<br><a href="/">去素材库看看</a>。</div>';return}el.innerHTML=list.map(collectionCard).join('')}
 function collectionCard(c){const count=Number.isFinite(Number(c.shot_count))?Number(c.shot_count):0;return '<div class="coll-card" data-coll="'+esc(c.id)+'"><button type="button" class="coll-head" data-action="toggle-collection" aria-expanded="false" aria-controls="coll-shots-'+esc(c.id)+'"><div class="coll-title"><b>'+esc(c.name)+'</b>'+(c.description?'<span class="coll-desc">'+esc(c.description)+'</span>':'')+'</div><span class="coll-meta">'+count+' 个镜头 · 时长 '+fmtDur(c.total_duration_ms||0)+' · 创建于 '+esc(new Date(c.created_at).toLocaleString('zh-CN'))+'</span></button><div class="coll-tools"><button class="ghost danger" data-action="delete-collection">删除收藏</button></div><div class="coll-shots" data-shots="'+esc(c.id)+'" hidden></div></div>'}
function collectionToggle(id){const box=document.querySelector('[data-shots="'+id+'"]');const card=document.querySelector('[data-coll="'+id+'"]');if(!box||!card)return;const head=card.querySelector('.coll-head');if(!box.hidden){box.hidden=true;card.classList.remove('open');if(head)head.setAttribute('aria-expanded','false');return}box.hidden=false;card.classList.add('open');if(head)head.setAttribute('aria-expanded','true');if(box.dataset.loaded)return;box.dataset.loaded='1';shotsLoad(id)}
...
async function copyTimecode(cid,i){const shot=shotCache[cid]&&shotCache[cid][i];if(!shot)return;const text=fmtClock(shot.start_ms)+'–'+fmtClock(shot.end_ms);try{await navigator.clipboard.writeText(text);collSay('已复制时间标记 '+text,true)}catch(e){const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();try{document.execCommand('copy');collSay('已复制时间标记 '+text,true)}catch(e2){collSay('复制失败：'+e2.message,false)}ta.remove()}}
