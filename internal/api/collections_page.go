...
const collectionsHTML = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Timingdex · [[i18n:collections.title]]</title><style>
...
async function collectionsLoad(){try{const list=await fetch('/api/v1/collections',{headers:authHeaders()}).then(async r=>{if(!r.ok)throw Error(await tdApiErrorMessage(r));return r.json()});collectionsRender(Array.isArray(list)?list:[])}catch(e){document.getElementById('collections').innerHTML='';collSay(tdT('collections.loadError',{message:e.message}),false)}}
function collectionsRender(list){const el=document.getElementById('collections');if(!list.length){el.innerHTML='<div class="coll-empty">'+esc(tdT('collections.empty'))+'<br><a href="/">'+esc(tdT('collections.empty.goLibrary'))+'</a></div>';return}el.innerHTML=list.map(collectionCard).join('')}
