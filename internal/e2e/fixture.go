...
// installs ffmpeg, so the full chain runs there. Set NEXUSGATE_MEDIA_OPTIONAL=1
...
func fixtureSkipOrFail(t testing.TB, message string) {
	t.Helper()
	if fixtureFailureShouldSkip(os.Getenv("NEXUSGATE_MEDIA_OPTIONAL"), os.Getenv("CI"), os.Getenv("GITHUB_ACTIONS")) {
		t.Skip(message)
	}
	t.Fatal(message)
}
