...
func TestMigrationSnapshotCreatedOnPendingMigrations(t *testing.T) {
	dir := t.TempDir()
	dbPath := filepath.Join(dir, "nexusgate.db")
	...
	files := preSnapshotFiles(t, dir, "nexusgate.db")
	if len(files) != 1 {
		t.Fatalf("Migrate on a fresh DB created %d snapshots, want 1: %v", len(files), files)
	}
	pattern := regexp.MustCompile(`^nexusgate\.db\.pre-\d+-\d{8}$`)
	...
	if !strings.HasPrefix(files[0], "nexusgate.db.pre-"+firstToken+"-") {
...
func TestMigrationSnapshotSkippedWhenItAlreadyExists(t *testing.T) {
	dir := t.TempDir()
	repo, err := Open(filepath.Join(dir, "nexusgate.db"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { repo.Close() })
	...
	before := preSnapshotFiles(t, dir, "nexusgate.db")
	if len(before) != 1 {
	...
	after := preSnapshotFiles(t, dir, "nexusgate.db")
...
func TestMigrationSnapshotNotCreatedWhenUpToDate(t *testing.T) {
	...
	if files := preSnapshotFiles(t, dir, "nexusgate.db"); len(files) != 0 {
...
func TestMigrationSnapshotUsesConsistentWALState(t *testing.T) {
	...
	removeNewestMigration(t, repo)
	for _, path := range preSnapshotFiles(t, dir, "nexusgate.db") {
