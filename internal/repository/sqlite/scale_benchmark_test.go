...
const (
	...
	scaleEmbeddingModel = "timingdex-scale-embedding-v1"
)
...
func benchSQLiteScale(b *testing.B, shots int, freshOpen bool, scenario scaleScenario) {
	...
	if !scaleSizeEnabled(shots) {
		b.Skipf("size %d is disabled; set TIMINGDEX_SQLITE_SCALE_SIZES to enable it", shots)
	}
...
func (scaleEmbedder) Name() string  { return "timingdex-scale-embedder" }
...
func scaleSizeEnabled(shots int) bool {
	value := strings.TrimSpace(os.Getenv("TIMINGDEX_SQLITE_SCALE_SIZES"))
	if value == "" {
		return shots == 1_000
	}
	if value == "all" {
...
func seedScaleCorpus(ctx context.Context, repo *Repository, shots int) error {
	...
	rootID := "scale-root"
	if _, err := tx.ExecContext(ctx, `INSERT INTO library_roots(id,path,created_at,updated_at) VALUES(?,?,?,?)`, rootID, "/timingdex-scale-corpus", now, now); err != nil {
		return err
	}
	...
	for i := 0; i < shots/scaleShotsPerAsset; i++ {
		...
		if _, err := locationStmt.ExecContext(ctx, "scale-location-"+strconv.Itoa(i), assetID, rootID, fmt.Sprintf("clips/scale-%07d.mp4", i), fmt.Sprintf("/timingdex-scale-corpus/clips/scale-%07d.mp4", i), now); err != nil {
			return err
		}
