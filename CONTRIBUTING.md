# Contributing to Re:Footage (`nexusgate`)
...
Repository: <https://github.com/evjohn-icu/nexusgate>
Module path: `github.com/evjohn-icu/nexusgate`
...
- `skills/nexusgate/` is a versioned agent-facing contract. If you change the API
  surface, keep `GET /api/v1/agent/capabilities` and
  `skills/nexusgate/references/api-contract.md` in sync — the skill relies on
...
./nexusgate root scan <root-id>     # scans, enqueues, and drains the Pipeline
./nexusgate pipeline run
./nexusgate serve
```

All Hub state is under `$NEXUSGATE_DATA_DIR` (`nexusgate.db`, optional
