...
- `nexusgate-hub.xml` — the database, HTTPS API and browser UI.
- `nexusgate-worker.xml` — a paired node that only runs FFmpeg derive work.
...
at `nexusgate-hub.xml` (via **Template repositories** if you host this repo's
...
```sh
mkdir -p /mnt/user/appdata/nexusgate-hub
...
clean `nexusgate-hub` appdata directory boots as-is and every admin/mutating
...
template variables — `NEXUSGATE_HUB_ADMIN_AUTH` = `trusted_network` and
`管理信任网段 NEXUSGATE_HUB_ADMIN_AUTH_NETWORKS` = your real client CIDRs (e.g.
...
nexusgate-hub` shows no admin-auth refusal). Read the generated `admin-token` from
...
2. **Point the media path at the `/mnt/remotes` parent, not one specific
   ...
   `/mnt/remotes/<server>_<share>`. NexusGate still records whatever
...
4. The one-time `chown -R 10001:10001 /mnt/user/appdata/nexusgate-hub` (and
   the matching one for `nexusgate-worker`) described above is unaffected by
...
/var/lib/nexusgate-worker/worker.json`, and that file only exists after
...
Add the `nexusgate-worker.xml` template but run the enrollment command
...
```sh
mkdir -p /mnt/user/appdata/nexusgate-worker
chown -R 10001:10001 /mnt/user/appdata/nexusgate-worker
...
`--config /var/lib/nexusgate-worker/worker.json` is mandatory: the Worker's
default config path is `~/.nexusgate/worker.json` (not its data directory),
and the template's PostArgs run `worker run --config
/var/lib/nexusgate-worker/worker.json`. Enroll and run must agree on the same
path, and that path must live on the persistent data directory
(`/mnt/user/appdata/nexusgate-worker`) so a container recreate does not lose
the pairing.

Once it succeeds, start the `nexusgate-worker` container normally.
...
```sh
docker exec nexusgate-hub nexusgate doctor
docker exec nexusgate-worker nexusgate worker doctor
docker exec nexusgate-hub vainfo      # only present in the gpu image variant
```

`nexusgate worker doctor` is a local hardware/enrollment report (Hub URL,
...
Back up the Hub data directory before upgrading: stop the `nexusgate-hub`
